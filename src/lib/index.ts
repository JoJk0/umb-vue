import { type UmbElement, UmbElementMixin } from '@umbraco-cms/backoffice/element-api'
import { customElement } from '@umbraco-cms/backoffice/external/lit'
import { extend, isPlainObject } from '@vue/shared'
import { type ComponentObjectPropsOptions, type ComponentOptions, type CreateAppFunction, type CustomElementOptions, type DefineComponent, type RenderFunction, type SetupContext, Suspense, VueElement, type VueElementConstructor, getCurrentInstance, useHost } from 'vue'
 ...
 */
export function defineManifest(options: Parameters<typeof umbExtensionsRegistry.register>[0] & {
  /**
 ...
 */
export function defineUmbVueElement(options: any, extraOptions?: ComponentOptions,
  /**
   * @internal
   */
  _createApp?: CreateAppFunction<Element>): HTMLElementConstructor<UmbElement> & VueElementConstructor {
  ...
  const Comp = defineComponent(isAsyncSetup
    ? {
      ...
      render() {
        return h(Suspense, {}, {
          default: () => h(options, { ...(this as unknown as VueInstance).$attrs, ...(this as unknown as VueInstance).$props }),
        })
      },
  ...
  class UmbVueCustomElement extends UmbElementMixin(VueElement) {
    ...
    constructor(initialProps?: Record<string, any>) {
      ...
      options.emits?.forEach((emit: string) => {
        if (emit.startsWith('update:')) {
          this.addEventListener(emit, (e) => {
            (this as Record<string, any>)[emit.replace('update:', '')] = (e as CustomEvent).detail[0]
      ...
      this.setAttribute('class', [this.getAttribute('class') ?? '', UMB_VUE_ELEMENT_CLASS].join(' '))
    }
  }
 ...
 */
export function useRepository<TRepository extends new (...args: any[]) => any>(Repository: TRepository) {
  const hostEl = useHostElement() as Record<string, any> | null

  if (!hostEl)
    throw new Error('[umb-vue]: `useRepository` must be called within a Vue Custom Element')
  ...
  return prototypeProperties as TRepository extends new (...args: any[]) => infer T ? T : never
}
 ...
 */
export async function useContext<T extends UmbContext>(CONTEXT_TOKEN: UmbContextToken<T>, host?: UmbClassInterface & UmbElement) {
  // Host element is mandatory if calling from an async function
  // https://4ark.me/post/87ba8d8b.html/
  const hostEl = (host ?? useHostElement()) as UmbClassInterface & UmbElement & {
    [k in typeof CONTEXT_TOKEN.contextAlias]?: T
  }

  if (!hostEl)
    throw new Error('[umb-vue]: `useContext` must be called within a Vue Custom Element. If you are calling it within a async function, pass the host element as the second argument.')

  if (!hostEl[CONTEXT_TOKEN.contextAlias])
    hostEl[CONTEXT_TOKEN.contextAlias] = await consumeContext(hostEl as UmbClassInterface & UmbElement, CONTEXT_TOKEN)

  const prototypeProperties = getAllMethods(hostEl[CONTEXT_TOKEN.contextAlias]!)

  const reactiveProperties = elObservablesToRefs(hostEl[CONTEXT_TOKEN.contextAlias]!)

  return { ...hostEl[CONTEXT_TOKEN.contextAlias], ...prototypeProperties as Record<string, never>, ...reactiveProperties }
}
 ...
 */
export function useHostElement<T extends UmbClassInterface & UmbElement = UmbClassInterface & UmbElement>() {
  const instance = getCurrentInstance() as { ce?: T, parent?: { ce?: T } }

  if (instance?.ce)
    return instance.ce
  else if (instance?.parent?.ce)
    return instance.parent.ce
  else
    throw new Error('[umb-vue]: `useHostElement` must be called within a Umbraco Vue Custom Element')
}

import { type BaseOptions, FilterFileType, type MarkRequired, detectVueVersion, getFilterPattern, isCallOf } from '@vue-macros/common'
...
export function filterManifestMacro(stmts: Statement[], test: string): CallExpression[] {
  return stmts
    .map((raw: Node) => {
      let node = raw
...
export function filterMacro(stmts: Statement[], literal: string): VariableDeclaration[] {
  return stmts
    .filter((node): node is VariableDeclaration => {
      const isVar = node.type === 'VariableDeclaration'
      if (isVar) {
        const decl = node.declarations[0]
        if (decl.init && isCallOf(decl.init, literal))
          return true
      }
      return false
    })
...
export function extractContextData(nodes: VariableDeclaration[]) {
  return nodes.map((node) => {
    ...
    const keys = id.type === 'ObjectPattern'
    ...
    return {
      keys: keys.findIndex(key => key === '*') !== -1 ? '*' as const : keys,
      contextToken: contextTokenArg.name
    }
  })
}

export function extractRepositoryData(nodes: VariableDeclaration[]) {
  return nodes.map((node) => {
...
export function checkDefaultExport(stmts: Statement[], key: string) {
...
export function getRootScopeIds(program: Program) {
  let ids: string[] = []
  walkAST(program, {
    enter(node) {
